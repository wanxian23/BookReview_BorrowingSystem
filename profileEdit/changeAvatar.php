        ...
        $allowedTypes = ['jpg', 'jpeg', 'png'];
         ...
         if (in_array($imageFileType, $allowedTypes) && $file["size"] <= 2 * 1024 * 1024) {
                if (move_uploaded_file($file["tmp_name"], $targetFile)) {
                    $updateSQL = "UPDATE Reader_User SET avatar = '$fileName' WHERE username = '$username'";
                    if ($conn->query($updateSQL)) {
                        echo "Avatar updated successfully! Redirecting...";
                        echo "<script>setTimeout(() => window.location.href = '/BookReview_BorrowingSystem/profile.php', 3000);</script>";
                    } else {
