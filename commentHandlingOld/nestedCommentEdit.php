...
<body>
    ...
    if ($_SERVER['REQUEST_METHOD'] == "POST") {
        ...
        $sql = "UPDATE Nested_Comment_Rating SET comment = '$editComment' WHERE nestedCommentCode = '$nestedCommentCode'";
        $runSQL = $conn->query($sql);

        if ($runSQL) {
            echo "<label class='output'>Nested Comment Edited Successfully! Back to post....</label>";
            ...
            echo "<label class='output'>Failed To Edit Nested Comment! Try Again!</label>";
            ...
            echo "<script>
                    setTimeout(function() {
                        window.location.href = '../nestedCommentEdit.php?postCode={$postCode}';
                    }, 3000);
                </script>";            
        }
    } else {
    ...
    <div class="edit-container">
        <div class="edit-header">
            <h2
                style="text-align: center; font-size: 1.6em;">Edit Nested Comment
            </h2>
        </div>

        <form class="formEdit" action="<?php echo htmlspecialchars("nestedCommentEdit.php?nestedCommentCode={$userComment['nestedCommentCode']}&postCode={$postCode}"); ?>" method="post">
            <p>
                Old Comment: <label for="" style="font-weight: normal;"><?php echo $userComment['comment']; ?></label>
            </p>

            <p>
                New Comment : <label for="" style="font-weight: normal;"><textarea type="text" placeholder="Reply" id="editComment" name="editComment" autofocus required><?php echo $userComment['comment']; ?></textarea></label>
            </p>
