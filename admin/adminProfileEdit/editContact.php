    ...
    <script>
        $(document).ready(function() {
            $("#confirmBtn").click(function(event) {
                ...
                if (contact === "") {
                    window.alert("Contact Number Cannot Be Null!");
...
<body>
    ...
    if ($_SERVER['REQUEST_METHOD'] == "POST") {
        ...
        $sqlCheck = "SELECT * FROM admin WHERE adminPhone = '$newContact'";
        $runSQLCheck = $conn->query($sqlCheck);

        $sqlUserCheck = "SELECT * FROM Reader_User WHERE phone = '$newContact'";
        $runSQLUserCheck = $conn->query($sqlUserCheck);

        if ($runSQLCheck->num_rows > 0 || $runSQLUserCheck->num_rows > 0) {
            echo "Fail to modify contact! Phone Already Exist! Please Try Again....";
            echo "<meta http-equiv='refresh' content='3; URL=../adminProfile.php'>";   
        } else {
            $sql = "UPDATE admin SET adminPhone = '$newContact' WHERE adminID = '$readerID'";
            $runSQL = $conn->query($sql);
    ...
    <main style="<?php echo $showPHPHandle ? 'display: none;' : 'display: block;'; ?>">
        <div class="edit-container">
            <div class="edit-header">
                <h2
                    style="text-align: center; font-size: 1.6em;">Edit Contact Number
                </h2>
            </div>

            <form class="formEdit" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>" method="post">
                <p>
                    Old Contact Number: <label for="" style="font-weight: normal;"><?php echo $admin['adminPhone']; ?></label>
                </p>

                <p>
                    New Contact Number: <label for="" style="font-weight: normal;"><input type="text" placeholder="New Contact Num" id="contact" name="contact" autofocus required></label>
                </p>
