...
<body>
    ...
    if ($_SERVER['REQUEST_METHOD'] == "POST") {
        ...
        $sql = "DELETE FROM Comment_Rating WHERE commentCode = '$commentCode'";
        $runSQL = $conn->query($sql);

        $sqlRating = "DELETE FROM Book_Borrowed WHERE postCode = '$postCode'";
        $runSQLRating = $conn->query($sqlRating);


        if ($runSQL && $runSQLRating) {
            echo "<label class='output'>Comment Deleted Successfully! Back to post....</label>";
            ...
            echo "<label class='output'>Failed To Delete Comment! Try Again!</label>";
            ...
            echo "<script>
                    setTimeout(function() {
                        window.location.href = '../ratingDelete.php?postCode={$postCode}';
                    }, 3000);
                </script>";            
        }
    } else {
    ...
    <div class="edit-container">
        ...
        <form class="formEdit" action="<?php echo htmlspecialchars("ratingDelete.php?commentCode={$userComment['commentCode']}&postCode={$postCode}"); ?>" method="post">
            <p style="font-size: 1.3em;">Are you sure you want to delete this comment & rating?</p>
