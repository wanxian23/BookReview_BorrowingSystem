...
<body>
    ...
    if ($_SERVER['REQUEST_METHOD'] == "POST") {
        ...
        $sqlGetCommentDetails = "SELECT *
                                FROM Nested_Comment_Rating
                                WHERE commentCode = '$commentCode' AND
                                readerID = '$readerID'
        ...
        if ($runSQL && $resultNotification) {
            ...
            echo "<label class='output'>Failed To Reply {$userComment['username']}! Try Again!</label>";
            ...
            echo "<script>
                    setTimeout(function() {
                        window.location.href = '../comemntReply.php?postCode={$postCode}';
                    }, 3000);
                </script>";            
        }
    } else {
    ...
    <div class="edit-container">
        <div class="edit-header">
            <h2
                style="text-align: center; font-size: 1.6em;">Reply '<?php echo $userComment['username']; ?>'
            </h2>
        </div>

        <form class="formEdit" action="<?php echo htmlspecialchars("commentReply.php?commentCode={$userComment['commentCode']}&postCode={$postCode}"); ?>" method="post">
            <p>
                Comment <?php echo $userComment['username']; ?>: <label for="" style="font-weight: normal;"><?php echo $userComment['comment']; ?></label>
            </p>

            <p>
                Reply to <?php echo $userComment['username']; ?>: <label for="" style="font-weight: normal;"><textarea type="text" placeholder="Reply" id="replyReader" name="replyReader" autofocus required></textarea></label>
            </p>
